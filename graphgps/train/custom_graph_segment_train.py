...
def train_epoch(logger, loader, model, optimizer, scheduler, emb_table, batch_accumulation):
    ...
    for iter, batch in enumerate(loader):
        ...
        for i in range(len(batch.y)):
            ...
            for j in range(num_parts):
                if j == segment_to_train:
                    ...
                    batch_train_list.append(data)
                else:
                    batch_other.append(emb_table.pull(batch_list[i].partition_idx+j))
        ...
        batch_train.to(torch.device(cfg.accelerator))
        for i, module in enumerate(model.children()):
            if i < module_len - 1:
                batch_train = module(batch_train)
            if i == module_len - 1:
                batch_train_embed = module.pooling_fun(batch_train.x, batch_train.batch)
        ...
        if len(batch_other) > 0:
            ...
            batch_other_embed = torch.zeros_like(batch_train_embed)
            part_cnt = 0
            ...
            graph_embed = (batch_train_embed*multiplier_num + batch_other_embed) / batch_num_parts
        else:
            graph_embed = batch_train_embed
        for i, module in enumerate(model.children()):
            if i == module_len - 1:
        ...
        if ((iter + 1) % batch_accumulation == 0) or (iter + 1 == len(loader)):
            ...
            optimizer.step()
            optimizer.zero_grad()
        for i in range(batch_train_embed.shape[0]):
            push_idx = batch_list[i].partition_idx+segments_to_train[i]
            emb_table.push(batch_train_embed[i].cpu(), push_idx)
...
@register_train('custom_graph_segment_train')
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    emb_table = History(60000, cfg.gnn.dim_inner)
